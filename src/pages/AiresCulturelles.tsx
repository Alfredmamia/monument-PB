...
import { Users, MapPin, Camera, Star, BookOpen, X } from 'lucide-react';
import Header from '@/components/Header';
import Footer from '@/components/Footer';
import { culturalAreaDetails } from '@/data/culturalAreasContent';

const AiresCulturelles = () => {
  const [selectedArea, setSelectedArea] = useState<string | null>(null);
  const activeDetail = selectedArea ? culturalAreaDetails[selectedArea] : null;
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <section className="relative h-96 bg-gradient-to-br from-amber-800 via-orange-700 to-red-800 flex items-center justify-center text-white overflow-hidden pattern-institutional">
        ...
        <div className="container mx-auto px-4">
          <div className="grid lg:grid-cols-2 gap-16">
            ...
            <Card className="overflow-hidden hover:shadow-2xl transition-shadow group">
              <div className="relative h-80 bg-gradient-to-br from-green-500 to-sky-600">
                <img 
              ...
              <CardContent className="p-8">
                ...
                <Button onClick={() => setSelectedArea('grassfield')} className="bg-green-600 hover:bg-green-700 text-white">
            ...
            <Card className="overflow-hidden hover:shadow-2xl transition-shadow group">
              <div className="relative h-80 bg-gradient-to-br from-sky-500 to-cyan-600">
                <img 
              ...
              <CardContent className="p-8">
                ...
                <Button onClick={() => setSelectedArea('sawa')} className="bg-sky-600 hover:bg-sky-700 text-white">
                  <BookOpen className="w-4 h-4 mr-2" />
                  Explorer cette Aire
                </Button>
              </CardContent>
            ...
            <Card className="overflow-hidden hover:shadow-2xl transition-shadow group">
              ...
              <CardContent className="p-8">
                ...
                <Button onClick={() => setSelectedArea('fangbeti')} className="bg-amber-600 hover:bg-amber-700 text-white">
                  <BookOpen className="w-4 h-4 mr-2" />
                  Explorer cette Aire
                </Button>
              </CardContent>
            ...
            <Card className="overflow-hidden hover:shadow-2xl transition-shadow group">
              ...
              <CardContent className="p-8">
                ...
                <Button onClick={() => setSelectedArea('soudano')} className="bg-orange-600 hover:bg-orange-700 text-white">
                  <BookOpen className="w-4 h-4 mr-2" />
                  Explorer cette Aire
                </Button>
              </CardContent>
      ...
      {/* Dialog — contenu expert exhaustif sur les objets d'art de l'aire culturelle sélectionnée */}
      <Dialog open={!!selectedArea} onOpenChange={(open) => !open && setSelectedArea(null)}>
        <DialogContent className="max-w-3xl max-h-[85vh] overflow-y-auto">
          {activeDetail && (
            <>
              <DialogHeader>
                <Badge className="w-fit mb-2 bg-national-gold text-white">Objets d'Art & Patrimoine</Badge>
                <DialogTitle className="text-2xl md:text-3xl font-bold text-gray-800">{activeDetail.name}</DialogTitle>
                <DialogDescription className="text-base text-gray-500 italic">{activeDetail.subtitle}</DialogDescription>
              </DialogHeader>

              <p className="text-gray-700 leading-relaxed border-l-4 border-national-gold pl-4 my-4">
                {activeDetail.intro}
              </p>
                ...
                {activeDetail.sections.map((section) => (
                  <div key={section.title}>
                    <h4 className="text-lg font-bold text-gray-800 mb-2 flex items-center gap-2">
                      <Star className="w-4 h-4 text-national-gold" />
                      {section.title}
                    </h4>
              ...
              <p className="text-xs text-gray-400 mt-6 pt-4 border-t">{activeDetail.sources}</p>
            </>
          )}
        </DialogContent>
      </Dialog>
